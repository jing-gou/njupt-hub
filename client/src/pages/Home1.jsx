...
export default function Home() {
    ...
    try {
      ...
      for (const r of items) {
        ...
        grouped[course].push({
          ...
          path: r.fileUrl,
          size: `${r.downloadCount ?? 0} 次下载`,
        });
      }

      setResources(grouped);
      localStorage.setItem('course_list', JSON.stringify(Object.keys(grouped)));
    } catch (error) {
  ...
  const handleDownload = async (file) => {
    try {
      if (file?.id) {
        fetch(`/api/resources/${file.id}/download`, { method: 'POST' });
      }
    } finally {
